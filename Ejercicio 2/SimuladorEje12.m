% ******************** MAPPING WITH GAUSSIAN MIXTURE MODEL ****************
...
for cont = 2:length(xx)-1
 ...
 %%   
    ...
    if(~isempty(MedPost) && ~isempty(Media_Label))
        ...
        Media_Label = identificar(MedPost,0.5);
    end 
%%
    % Gaussian Mixture Model for Mapping with a unicycle vehicle.
    % Covariance compute for every iteration and group.
    Ngauss = max(Media_Label(:,3));
    NumPost = max(Media_Label(:,3));
    for ng=1:NumPost
        seleccion = find(Media_Label(:,3) == ng);
        grp = Media_Label(seleccion',:);
        [f,c] = size(grp);
        if f>3 % Minimum value of Gaussian Mixture Model training.
            NumPostReal = NumPostReal+1;
            options = statset('Display','final');
            obj = fitgmdist(grp(:,1:2),1,'Options',options);
            covar = obj.Sigma; med = obj.mu;
            MedGMM = [MedGMM;grp];
            elipseGrid = elipse2(med,covar);
            plt_media(NumPostReal) = plot(med(1),med(2),'m*','LineWidth',2); % Mean at k iteration.
            titulo = strcat('Número de postes estimados: ',num2str(size(MAPA,1)));
            title(titulo)
            loglikelyhood = num2str(obj.NlogL);
            Iteraciones = num2str(obj.Iters);
            ejex = strcat('LogLike: ',loglikelyhood,',  Iter: ',Iteraciones);
            xlabel(ejex);
            hold on
            plt_eli(NumPostReal) = plot(elipseGrid(:,1),elipseGrid(:,2),'-k','LineWidth',1);
        end
    end
    Nt = NumPostReal;
    NumPostReal=0;
...
%%
    ...
    robot2.x = robot2.x + pasoTiempo*V*cos(robot2.tita)+0.015*rand(1,1);
    robot2.y = robot2.y + pasoTiempo*V*sin(robot2.tita)+0.015*rand(1,1);
...
M
for ng=1:NumPost
        seleccion = find(MedGMM(:,3) == ng);
        grp = MedGMM(seleccion',:);
        [f,c] = size(grp);
