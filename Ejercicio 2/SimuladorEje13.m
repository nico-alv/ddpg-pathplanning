% ********************* MAPPING WITH K MEANS METHODE  *********************
%** Implementacion FILTRO DE KALMAN ********************************
...
Media_Label = [];
NumPostReal = 0;
%% ****************  Using K-means to Identify characteristics in a MAP with a uniclycle vehicle. ********
for cont = 2:length(xx)-1
    ...
    BuscoPostes = [];  
    Caracteristicas = EncontrarCaracteristicas(Laser,robot);
 ...
 %%   
    ...
    for ng=1:NumPost
        ...
        if f>2 % Condition to get Covariance matrix
          ...
          [idx,media_grp] = kmeans(grp(:,1:2),1,'Distance','cityblock','Replicates',5,'Options',opts);
...
% MAP MATRIX DELIVERED: M, AND ITS COVARIANCE AT THE END OF THE PATH.
figure(2)
M
for ng=1:NumPost
        seleccion = find(Media_Label(:,3) == ng);
